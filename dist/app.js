(function(){
  ...
  // ===== 功能設定與複習計時狀態 =====
  // fx: {sfx:布林 音效, anim:布林 動效, vol:0~1 音量}
  if(!state.fx||typeof state.fx!=='object')state.fx={sfx:true,anim:true,vol:0.7};
  ...
  function sfxWrong(){tone(330,0.22,'sine',0,0.5);tone(262,0.28,'sine',0.14,0.45);}
  function sfxStreak(n){
    if(n>=20){tone(523,0.12,'triangle',0,1);tone(659,0.12,'triangle',0.1,1);tone(784,0.12,'triangle',0.2,1);tone(1047,0.3,'triangle',0.3,1);}
    else if(n>=10){tone(523,0.12,'triangle',0,1);tone(659,0.12,'triangle',0.12,1);tone(784,0.25,'triangle',0.24,1);}
    else if(n>=5){tone(659,0.12,'triangle',0,1);tone(880,0.25,'triangle',0.13,1);}
  }
  // ---- 答題動效 ----
  function fxBurst(ok){
    if(!state.fx.anim)return;
    const s=el('screen');if(!s)return;
    const r=s.getBoundingClientRect();
    const cx=r.width/2, cy=r.height*0.3;
    const colors=ok?['#07c160','#4aa8ff','#ffb454','#7bed9f','#fff']:['#ff6b6b','#ff9a9a','#8b9bb0'];
    for(let i=0;i<(ok?26:10);i++){
  ...
  function fxConfetti(){
    if(!state.fx.anim)return;
    const s=el('screen');if(!s)return;
    const colors=['#07c160','#4aa8ff','#ffb454','#c792ea','#ff6b6b','#7bed9f'];
    for(let i=0;i<40;i++){
      const p=document.createElement('div');
  ...
  // ---------- 複習月曆：每日時長 + 達標判定 + 週/月統計 ----------
  // 計劃章節 → 對應章號集合（如「第3章(上)」→[3]、「第1/2章補強」→[1,2]、「混合刷題」→全部[1..9]）
  function planChapters(chStr){
    const s=chStr||'';
    const nums=[];
    const m=s.match(/\d+/g)||[];
    m.forEach(x=>{const n=parseInt(x);if(n>=1&&n<=9)nums.push(n);});
  ...
  function calendarHTML(){
    ...
    for(let day=1;day<=dim;day++){
      ...
      if(ach)cls+=' cal-done';
      else if(mins>0)cls+=' cal-partial';
      cells+=`<div class="${cls}"><span class="cal-d">${day}</span>${mins>0?`<span class="cal-m">${Math.round(mins)}分</span>`:''}${ach?'<span class="cal-flag">✓</span>':''}</div>`;
    }
    const ms=monthStudy();
    return `<div class="card cal-card"><div class="ct">📅 複習月曆 · ${y} 年 ${m+1} 月</div>
      <div class="cal-legend"><span><i class="lg-done"></i>達標</span><span><i class="lg-partial"></i>有複習未達標</span><span><i class="lg-none"></i>未複習</span></div>
      <div class="cal-grid">${cells}</div>
      <div class="cal-sum">
        <span>本月複習 <b>${ms.mins}</b> 分鐘</span>
        <span>達標 <b>${ms.doneDays}</b> 天</span>
        <span>本週 <b>${weekStudyMins()}</b> 分鐘</span>
      </div>
      <div class="cal-note">達標規則：當日有複習，且所讀章節與計劃表當日章節相符。</div>
    </div>`;
  }
  function estHours(d){
    const s=(d.ch||'')+(d.t||'');
  ...
  function goNote(ch){
    ...
    if(!n){goNotes();return;}
    studyStartTimer(ch);  // 開啟講義 → 自動開始複習計時
    setScreen(`<div class="phead"><a data-act="notes" class="pback">‹ 講義</a> 第${ch}章講義
      <span class="study-timer-badge">📖 <span id="study-timer">0.0 分鐘</span></span></div>
  ...
  function goPractice(){
    curView='practice';studyStopTimer();
    setActive('practice');
  ...
  function goMe(){
    ...
    setScreen(`<div class="phead">我的</div>
      ...
      <div class="card">
        <div class="ct">🎮 答題反饋設定</div>
        <div class="fx-row"><span>✅ 答對動效（打勾／粒子）</span><label class="sw"><input type="checkbox" data-act="fx-anim" ${fx.anim?'checked':''}><i></i></label></div>
        <div class="fx-row"><span>🔊 答題音效</span><label class="sw"><input type="checkbox" data-act="fx-sfx" ${fx.sfx?'checked':''}><i></i></label></div>
  ...
  function submitAnswer(){
    ...
    if(!correct){if(!state.wrong.includes(key))state.wrong.push(key);fxStreak=0;}
    else{
      state.wrong=state.wrong.filter(k=>k!==key);
      fxStreak++;
      if(state.fx.sfx)sfxCorrect();
      if(state.fx.anim)fxBurst(true);
      if(fxStreak===5||fxStreak===10||fxStreak===20){
        if(state.fx.sfx)sfxStreak(fxStreak);
        if(state.fx.anim)fxConfetti();
        setTimeout(()=>toast('🔥 連續答對 '+fxStreak+' 題！彩蛋解鎖 🎉'),600);
      }
    }
    if(!correct&&state.fx.sfx)sfxWrong();
    save();renderQuiz();
  }
  function nextQ(){quiz.idx++;quiz.selections=[];if(quiz.idx>=quiz.questions.length)renderSummary();else renderQuiz();}
  function renderSummary(){
    curView='summary';studyStopTimer();
  ...
  document.addEventListener('click',e=>{
    ...
    if(act==='fx-anim'){state.fx.anim=a.checked;save();toast(state.fx.anim?'已開啟答對動效 ✨':'已關閉答對動效');return;}
    if(act==='fx-sfx'){state.fx.sfx=a.checked;save();if(state.fx.sfx)sfxCorrect();toast(state.fx.sfx?'已開啟音效 🔊':'已關閉音效 🔇');return;}
    if(act==='fx-vol'){const v=parseInt(a.value)/100;state.fx.vol=v;save();const vv=document.querySelector('.fx-vol');if(vv)vv.textContent=Math.round(v*100)+'%';if(state.fx.sfx)sfxCorrect();return;}
