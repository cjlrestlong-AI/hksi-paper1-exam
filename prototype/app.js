(function(){
  ...
  function pushSync(){
    ...
    fetch((syncBase||'')+'/api/progress',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uid:syncUid,data:state,updatedAt:Date.now()})})
      .then(r=>r.ok?r.json():null).then(j=>{ if(j&&j.updatedAt)state._syncedAt=j.updatedAt; })
      .catch(()=>{}).then(()=>{pushing=false;});
  }
  function mergeState(R){
    const L=state; L.answered=L.answered||{}; R.answered=R.answered||{};
    const a={...L.answered};
    Object.keys(R.answered).forEach(k=>{ if(!a[k]||(R.answered[k].ts||0)>=(a[k].ts||0)) a[k]=R.answered[k]; });
    L.answered=a;
    ['wrong','fav'].forEach(f=>{ const s=new Set((L[f]||[]).concat(R[f]||[])); L[f]=[...s]; });
    ['checkins','planDone'].forEach(f=>{ const s=new Set((L[f]||[]).concat(R[f]||[])); L[f]=[...s]; });
    save();
  }
  function pullSync(cb){
    ...
    fetch((syncBase||'')+'/api/progress?uid='+encodeURIComponent(syncUid)).then(r=>r.ok?r.json():null).then(j=>{
      if(j&&j.data){ mergeState(j.data); if(curTab==='home')goHome(); }
      else { pushSync(); }
      cb&&cb();
    }).catch(()=>{cb&&cb();});
  }
  function initSync(){
    ...
    fetch((syncBase||'')+'/api/health').then(r=>r.ok?r.json():null).then(j=>{
      if(j&&j.ok){                // 同網域同步伺服器可用 → 啟用雲端同步
        const u=getUid();
        if(u){ syncUid=u; pullSync(); }
        else { showSyncSetup(false); }
      } else {
        syncOn=false;             // 靜態部署（無伺服器）：優雅停用雲端 UI，避免彈出無效設定框
      }
    }).catch(()=>{ syncOn=false; });
  }
  function showSyncSetup(linkMode){
    ...
    if(baseSet)baseSet.onclick=()=>{
      ...
      fetch(b+'/api/health').then(r=>r.ok?r.json():null).then(j=>{
        if(j&&j.ok){ setBase(b); syncBase=b; berr.textContent=''; if(getUid()){syncUid=getUid();pullSync();} ov.remove(); toast('已連線同步伺服器 ☁️'); }
  ...
  function goMe(){
    ...
    setScreen(`<div class="phead">我的</div>
      ...
      <div class="card">
        ${syncOn?`
        <div class="mlink" data-act="sync-settings"><span>☁️ 雲端同步</span><b>${syncUid?'已連線':'未啟用'} ›</b></div>
        <div class="mlink" data-act="sync-now"><span>🔄 立即同步上傳</span><b>›</b></div>
        ${syncUid?'<div class="sync-code">同步碼：<code>'+esc(syncUid)+'</code> <span class="sync-copy" data-act="sync-copy">複製</span></div>':''}
        `:`
        <div class="mlink disabled"><span>☁️ 雲端同步</span><b>此部署未啟用</b></div>
        <div class="sync-note">此為靜態部署，未連接同步伺服器。啟用跨裝置同步請執行 <code>server.js</code>（見部署說明）。</div>
        `}
      </div>
      ...
      <div class="about">
        ...
        <div>${syncOn&&syncUid?'進度已統一儲存於雲端伺服器，跨裝置不流失。':(syncOn?'已設定同步碼，下次進入將自動同步。':'未連接雲端同步伺服器，進度僅存本機，清除快取會遺失。')}</div>
      </div>`);
  }
  function goWrong(){
    setActive('me');
  ...
  document.addEventListener('click',e=>{
    ...
    if(act==='sync-settings'){ if(!syncOn){toast('此部署未啟用雲端同步');return;} if(syncUid){showSyncSetup(true);}else{showSyncSetup(false);} return; }
    if(act==='sync-now'){ if(!syncOn){return;} pushSync(); toast('已上傳至雲端 ☁️'); return; }
